...
import type { KnifeCategory } from "@/data/knives"

export type KnivesFilterState = {
  category: "all" | KnifeCategory
  steel: "all" | string
  handle: "all" | string
  price: [number, number]
...
export default function KnivesFilters({
  state,
  onChange,
  steels,
  handles,
  ...
  state: KnivesFilterState
  onChange: (next: KnivesFilterState) => void
  steels: string[]
  handles: string[]
    ...
    <div className="grid grid-cols-1 gap-4 rounded-lg border bg-card p-4 md:grid-cols-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <Select
          ...
          onValueChange={(v) => onChange({ ...state, category: v as KnivesFilterState["category"] })}
        >
          <SelectTrigger aria-label="Pilih jenis pisau">
